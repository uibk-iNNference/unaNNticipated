...
@cpp.command(name="contract")
@click.option("--output_path", type=click.Path(), default=None)
def calculate_cpp_contraction(output_path: str = None):

    samples = np.load(os.path.join("data", "datasets", "minimal", "samples.npy"))
    sample = samples[0]
    fixed_sample = np.concatenate(
        [sample[:, :, c] for c in range(2)]
    ...
    with tf.device("/device:CPU:0"):
        ...
        model = models.get_model("minimal")
        kernel = model.layers[-1].get_weights()[0]
        fixed_kernel = np.concatenate([kernel[:, :, c, 0] for c in range(2)])

        import forennsic_cpp

        contracted = forennsic_cpp.contract(
            fixed_sample.flatten(), fixed_kernel.flatten()
        )

    full_dict = dict()
    full_dict["cpu_info"] = metadata.get_cpu_info()
    full_dict["hostname"] = metadata.get_hostname()
    full_dict["prediction"] = metadata.convert_np_to_json(contracted)
    full_dict["model_type"] = "minimal"
    # full_dict["executing_commit"] = metadata.get_commit()
...
@simd.command()
@click.argument("a_path", type=str)
@click.argument("b_path", type=str)
@click.option("--output_path", type=click.Path(), default=None)
def element_wise(a_path: str, b_path: str, output_path: str=None):
    a = np.load(a_path)
    b = np.load(b_path)
    ...
    assert a.shape == (16,1)
    result = forennsic_cpp.mul_128(a,b)
    
    full_dict = dict()
    full_dict["cpu_info"] = metadata.get_cpu_info()
    full_dict["hostname"] = metadata.get_hostname()
    full_dict["prediction"] = metadata.convert_np_to_json(result)
    # full_dict["executing_commit"] = metadata.get_commit()
