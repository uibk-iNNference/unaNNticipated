...
CIFAR_VARIANTS = ["_sigmoid_untrained"]
...
def prepare(
    ...
    instrumental_model.save(join(model_dir, f"{instrumental_model.name}.h5"))

    # prepare inputs
    inputs = np.load(sample_path)
    for i in range(3):
        current_inputs = inputs[i : i + 1]
        np.save(join(sample_dir, f"{instrumental_model.name}-i{i}.npy"), current_inputs)
...
def main(clean: bool, upload: bool, stop: bool):
    ...
    for model, i in itertools.product(MODEL_CONFIGS, range(3)):
        instrumental_name = f"{model}_instrumental"
